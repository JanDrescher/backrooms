import { MeshBuilder, StandardMaterial, DynamicTexture, Texture, PointLight, Color3, Vector3, type Scene } from "@babylonjs/core";
...
export class PlaceholderRoom extends RoomBase {
  ...
  constructor(id: string, width = 6, depth = 6, height = 2.8) {
    super();
    this.id = id;
    this.W  = width;
    this.D  = depth;
    ...
    this.doors = [
      { id: "north", position: new Vector3(0, DOOR_H / 2, -this.D / 2), direction: new Vector3(0, 0, -1) },
  ...
  protected async buildGeometry(scene: Scene): Promise<void> {
    ...
    const wallMatN     = this.mat(scene, "wall_n",    Color3.White());
    ...
    const baseboardMat = this.mat(scene, "baseboard", new Color3(0.79, 0.77, 0.47));
    const corniceMat   = this.mat(scene, "cornice",   new Color3(0.57, 0.56, 0.48));

    for (const m of [floorMat, ceilMat, wallMatN, wallMatS, wallMatEW, wallMatNTop, frameMat, baseboardMat, corniceMat]) {
      m.maxSimultaneousLights = 5;
    }

    // Tapeten — physikalisch identische Kachelgröße auf allen Wänden
    wallMatN.diffuseTexture    = this.buildWallpaperTexture(scene, "n",     sideW / TILE_W,  wallH / TILE_H);
    ...
    // Nordwand mit Türöffnung (3 Segmente)
    const northLeft = MeshBuilder.CreateBox(`${this.id}_wall_north_L`,
      { width: sideW, height: wallH, depth: T }, scene);
    northLeft.position = new Vector3(-(DOOR_W / 2 + sideW / 2), wallH / 2, northZ);
    northLeft.material = wallMatN;
    this.track(northLeft);

    const northRight = MeshBuilder.CreateBox(`${this.id}_wall_north_R`,
      { width: sideW, height: wallH, depth: T }, scene);
    northRight.position = new Vector3(DOOR_W / 2 + sideW / 2, wallH / 2, northZ);
    northRight.material = wallMatN;
    this.track(northRight);

    const northTop = MeshBuilder.CreateBox(`${this.id}_wall_north_T`,
      { width: DOOR_W, height: topH, depth: T }, scene);
    northTop.position = new Vector3(0, DOOR_H + topH / 2, northZ);
  ...
  private buildDividers(scene: Scene): void {
    ...
    const H_DIV = H * 0.65;  // Stubhöhe — Spieler (1.7m) kann nicht drüberschauen
    const T_DIV = 0.15;      // Stubstärke
    const sectX = W / 3;
    const sectZ = D / 3;

    // Säulen: gleiche Farbe wie Scheuerleiste — hell, baulich
    const colMat = this.mat(scene, "div_col", new Color3(0.79, 0.77, 0.47));
    colMat.maxSimultaneousLights = 5;
    ...
    for (let ix = 1; ix < sectX; ix++) {
      for (let iz = 1; iz < sectZ; iz++) {
        ...
        // Stub: 0 oder 1 pro Säule, nur für äußere Säulen (nächste Außenwand ≤3.5m)
        ...
        // Bei gleich-nahen Außenwänden (Ecksäulen) zufällig wählen
        ...
        if (dir === 'N' || dir === 'S') {
          ...
          stub.position = new Vector3(colX, H_DIV / 2,
            colZ + signZ * (COL_S / 2 + stubLen / 2));
          stub.material = matNS;
          this.track(stub);
        } else {
          ...
          stub.position = new Vector3(
            colX + signX * (COL_S / 2 + stubLen / 2), H_DIV / 2, colZ);
          stub.material = matEW;
          this.track(stub);
        }
      }
  ...
  private buildBaseboards(scene: Scene, mat: StandardMaterial): void {
    ...
    for (const b of [
      ...
      { s: "NL", pos: new Vector3(-(DOOR_W / 2 + sideW / 2), BS_H / 2, -D / 2 + BS_D / 2), w: sideW, d: BS_D },
      { s: "NR", pos: new Vector3( DOOR_W / 2 + sideW / 2,   BS_H / 2, -D / 2 + BS_D / 2), w: sideW, d: BS_D },
  ...
  private buildDoor(scene: Scene, wallZ: number): void {
    ...
    const panel = MeshBuilder.CreateBox(`${this.id}_door_panel`,
      { width: DOOR_W - GAP * 2, height: DOOR_H - GAP, depth: THICK }, scene);
    panel.position = new Vector3(0, (DOOR_H - GAP) / 2, wallZ);
    panel.material = panelMat;
    this.track(panel);

    const handleMat = this.mat(scene, 'door_handle', new Color3(0.42, 0.38, 0.24));
    const faceZ = wallZ + THICK / 2;
    const HX = DOOR_W / 2 - 0.13;
    const HY = 1.05;

    const plate = MeshBuilder.CreateBox(`${this.id}_door_plate`,
      { width: 0.038, height: 0.16, depth: 0.007 }, scene);
    plate.position = new Vector3(HX, HY, faceZ + 0.0035);
    plate.material = handleMat;
    this.prop(plate);

    const ARM_D = 0.045, GRIP_W = 0.10, BEND_R = 0.018, TUBE_R = 0.007;
    const HH = HY + 0.038;
    const path: Vector3[] = [];
    path.push(new Vector3(HX, HH, faceZ + 0.007));
    path.push(new Vector3(HX, HH, faceZ + 0.007 + ARM_D));
    const bcX = HX - BEND_R, bcZ = faceZ + 0.007 + ARM_D;
    for (let i = 1; i <= 10; i++) {
      const a = (i / 10) * (Math.PI / 2);
      path.push(new Vector3(bcX + BEND_R * Math.cos(a), HH, bcZ + BEND_R * Math.sin(a)));
    }
    path.push(new Vector3(HX - BEND_R - GRIP_W, HH, bcZ + BEND_R));
  ...
  private buildDoorFrame(scene: Scene, wallZ: number, mat: StandardMaterial): void {
    ...
    fL.position = new Vector3(-DOOR_W / 2 + frameT / 2, DOOR_H / 2, wallZ);
    fL.material = mat;
    this.prop(fL);

    const fR = MeshBuilder.CreateBox(`${this.id}_frame_R`,
      { width: frameT, height: DOOR_H, depth: frameD }, scene);
    fR.position = new Vector3(DOOR_W / 2 - frameT / 2, DOOR_H / 2, wallZ);
    fR.material = mat;
    this.prop(fR);

    const fT = MeshBuilder.CreateBox(`${this.id}_frame_T`,
      { width: DOOR_W - frameT * 2, height: frameT, depth: frameD }, scene);
    fT.position = new Vector3(0, DOOR_H - frameT / 2, wallZ);
    fT.material = mat;
    this.prop(fT);
  }
}
